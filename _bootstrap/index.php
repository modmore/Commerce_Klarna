...
if (!createObject('modNamespace',array(
    'name' => 'commerce_klarna',
    'path' => $componentPath.'/core/components/commerce_klarna/',
    'assets_path' => $componentPath.'/assets/components/commerce_klarna/',
),'name', false)) {
    echo "Error creating namespace commerce_klarna.\n";
}

/* Path settings */
if (!createObject('modSystemSetting', array(
    'key' => 'commerce_klarna.core_path',
    'value' => $componentPath.'/core/components/commerce_klarna/',
    'xtype' => 'textfield',
    'namespace' => 'commerce_klarna',
    'area' => 'Paths',
    'editedon' => time(),
), 'key', false)) {
    echo "Error creating commerce_klarna.core_path setting.\n";
}

if (!createObject('modSystemSetting', array(
    'key' => 'commerce_klarna.assets_path',
    'value' => $componentPath.'/assets/components/commerce_klarna/',
    'xtype' => 'textfield',
    'namespace' => 'commerce_klarna',
    'area' => 'Paths',
    'editedon' => time(),
), 'key', false)) {
    echo "Error creating commerce_klarna.assets_path setting.\n";
}
...
$assetsUrl = "{$requestUri}assets/components/commerce_klarna/";

if (!createObject('modSystemSetting', array(
    'key' => 'commerce_klarna.assets_url',
    'value' => $assetsUrl,
    'xtype' => 'textfield',
    'namespace' => 'commerce_klarna',
    'area' => 'Paths',
    ...
    echo "Error creating commerce_klarna.assets_url setting.\n";
}


$settings = include dirname(__FILE__, 2) .'/_build/data/settings.php';
foreach ($settings as $key => $opts) {
    ...
    if (!createObject('modSystemSetting', array(
        ...
        echo "Error creating commerce_klarna.".$key." setting.\n";
    }
}
...
include $componentPath . '/core/components/commerce_klarna/vendor/autoload.php';

// Grab the path to our namespaced files
$modulePath = $componentPath . '/core/components/commerce_klarna/src/Modules/';

// Instruct Commerce to load modules from our directory, providing the base namespace and module path twice
$commerce->loadModulesFromDirectory($modulePath, 'modmore\\Commerce_Klarna\\Modules\\', $modulePath);
